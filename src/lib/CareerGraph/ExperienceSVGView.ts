...
class ExperienceSVGView {
		...
		render: function () {
			...
			this._renderSection(this.expWork, 'Work Experience')._renderSection(
				this.expProjects,
				'Personal Projects',
			);
		...
		_renderSection: function (collection, title) {
			this._renderHeader(title)._renderExperience(collection);
			...
			_.each(
				...
				function (exp) {
					var x = this.xRegular,
						y = getY(),
						viewClass =
							exp instanceof ExperienceWorkModel
								? ExperienceWorkView
