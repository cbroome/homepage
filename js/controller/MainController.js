define(function (require) {
	...
	MainController = Marionette.Controller.extend({
		...
		initialize: function () {
			...
			// Reset selected
			this.listenTo(this.projects, 'change:selected', this._resetSelected);

			this.listenTo(this.jobs, 'change:selected', this._resetSelected);

			this.listenTo(this.skills, 'change:selected', this._resetSelected);
		...
		_resetSelected: function (model) {
			var collections = [this.projects, this.jobs, this.skills],
		...
		_hoverEnd: function (model) {
			...
			app.vent.trigger(EVENTS.PATHS.RESET);
			_.each(exps, this._reselectExperience, this);
		},
